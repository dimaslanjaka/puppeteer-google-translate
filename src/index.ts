...
function translateText(text: string | string[], opt: Options): Promise<string | string[]> {
	if (typeof text === 'string')
		return fromText(buildQuery({ text, to: opt.to, from: opt.from, op: 'translate' }), {
...
function translateDocs(path: string | string[], opt: Options): Promise<string | string[]> {
	if (typeof path === 'string') {
		return fromDocs(buildQuery({ to: opt.to, from: opt.from, op: 'docs' }), path, {
			headless: opt.headless,
			timeout: opt.timeout
		});
	}
...
export { translateText, translateDocs, Options as PuppeteerTranslateOptions };
